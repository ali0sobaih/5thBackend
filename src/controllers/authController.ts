...
const register = async (req: Request, res: Response) => {
    try {
        const { first_name, last_name, email, phone, password } = (req as any).validated as UserRegister;
        ...
        return errorResponse(res, "An error occurred during registration", error, 500);
    }
};
...
interface AuthenticatedRequest extends Request {
  ...
  const logout = async (req: AuthenticatedRequest, res: Response) => {
    try {
      ...
      return errorResponse(res, "Failed to logout", (error as Error).message, 500);
    }
  };
